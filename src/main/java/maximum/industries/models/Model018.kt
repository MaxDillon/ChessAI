...
/**
 * Shallower, but starting off with 5x5 convolutions, and ending with 1x1 for policy layer instead of 3x3
 */
class Model018 : IModel {
    override fun newModel(gameSpec: GameGrammar.GameSpec,
        ...
        val config = NeuralNetConfiguration.Builder()
                ...
                // ######## First convolution #############################
                .F("conv1", "input") {
                    convolution(inChannels, filters = convFilters, kernel = 5, pad = 2, init = WeightInit.XAVIER, activation = activation,
                                norm = false)
                }

                // ######## Residual block    #############################
                .R("res1", "conv1") {
                    convolution(filters = convFilters, kernel = 5, pad = 2, init = weightInit, activation = activation)
                    convolution(filters = convFilters, kernel = 5, pad = 2, init = weightInit, activation = activation)
                }
                .R("res2", "res1") {
                    convolution(filters = convFilters, kernel = 5, pad = 2, init = weightInit, activation = activation)
                ...
                // ######## Policy head      #############################
                .F("policy1", "res4") {
                    convolution(filters = policyChannels(gameSpec), kernel = 1, pad = 0, init = weightInit, activation = activation)
                }
